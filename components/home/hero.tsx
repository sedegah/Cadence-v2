...
export default function Hero() {
  ...
  const fullSubtext = "Software & Web Development"
    ...
    if (textComplete) {
      ...
      const subtextInterval = setInterval(() => {
        if (currentIndex <= fullSubtext.length) {
          setDisplaySubtext(fullSubtext.substring(0, currentIndex))
          currentIndex++
        } else {
      ...
      <div className="relative z-10 container mx-auto px-4 md:px-6">
        <div className="max-w-3xl mx-auto text-center">
          ...
          >
            ...
            <span className="text-sm tracking-wide text-white/60">Code Cadence</span>
          </motion.div>
          ...
          <motion.div custom={2} variants={fadeUpVariants} initial="hidden" animate="visible">
            <p className="text-base sm:text-lg md:text-xl text-white/40 mb-8 leading-relaxed font-light tracking-wide max-w-xl mx-auto px-4">
              {displaySubtext}
              {!subtextComplete && <span className="animate-pulse">|</span>}
            </p>
          </motion.div>
          ...
          >
            ...
            <Link
              href="/projects"
              className="px-6 py-3 rounded-md bg-transparent hover:bg-white/10 text-white border border-white/20 font-medium transition-colors"
            >
              View Projects
            </Link>
          </motion.div>
